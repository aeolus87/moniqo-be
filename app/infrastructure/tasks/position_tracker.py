...
class PositionTrackerService:
    ...
    def __init__(self):
        ...
        Uses db_provider.get_db() for all database access to ensure
        automatic routing based on trading mode context.
        """
    ...
    async def _is_flow_active(self, position: Position) -> bool:
        ...
        try:
            ...
            flow = await flow_service.get_flow_by_id(db, str(position.flow_id))
            if not flow:
    ...
    async def _insert_position_update(
        ...
        try:
            db = db_provider.get_db()
            await db["position_updates"].insert_one({
        ...
        try:
            ...
            if not position:
                ...
                if flow_id:
                    try:
                        from app.modules.flows import service as flow_service
                        flow = await flow_service.get_flow_by_id(db, str(flow_id))
                ...
                if user_wallet_id:
                    try:
                        wallet_instance = await create_wallet_from_db(db, str(user_wallet_id))
                ...
                try:
                    ...
                    if "app.main" in sys.modules:
                        ...
                        if not user_id:
                            ...
                            if flow_id:
                                try:
                                    flow_doc = await db["flows"].find_one({"_id": ObjectId(flow_id)})
                                    if flow_doc:
            ...
            if position.user_wallet_id:
                try:
                    db = db_provider.get_db()
                    wallet_instance = await create_wallet_from_db(db, str(position.user_wallet_id))
                ...
                for rec in monitor_result.get("recommendations", []):
                    if rec.get("position_id") == str(position.id) and rec.get("action") in ["close", "exit"]:
                        ...
                        if refreshed_position.user_wallet_id:
                            try:
                                db = db_provider.get_db()
                                wallet_instance = await create_wallet_from_db(db, str(refreshed_position.user_wallet_id))
    ...
    async def _record_learning_outcome(
        ...
        try:
            ...
            db = db_provider.get_db()
            await db["learning_outcomes"].insert_one(learning_record)
            ...
            if position.user_id and position.user_wallet_id and not position.simulated:
                try:
                    ...
                    circuit_breaker = get_circuit_breaker_service(db)
                    ...
                    daily_tracker = get_daily_loss_tracker(db)
    ...
    async def _trigger_flow_continuation(self, flow_id: str) -> None:
        ...
        try:
            ...
            await flow_service._schedule_auto_loop(db, flow, "groq", None)
        except Exception as e:
...
async def get_position_tracker() -> PositionTrackerService:
    """
    Get global position tracker service instance.
    
    Uses db_provider.get_db() internally for all database access,
    ensuring automatic routing based on trading mode context.
    ...
    if _position_tracker is None:
        _position_tracker = PositionTrackerService()
