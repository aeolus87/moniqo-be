...
from app.infrastructure.tasks.trading_mode_helpers import get_trading_mode_from_flow
...
async def _trigger_flow(flow_id: str, model_provider: str = "groq", trading_mode: TradingMode = None):
    ...
    if trading_mode is None:
        trading_mode = await get_trading_mode_from_flow(flow_id)
    
    # Set context for flow execution
...
def trigger_scheduled_flows_task():
    ...
    Processes flows from both demo and real databases.
    """
    ...
    async def run():
        ...
        for flow_doc in flows:
            ...
            if _should_run_now(schedule):
                ...
                logger.info(f"Triggering flow {flow_doc.get('name')} - schedule: {schedule} (mode: {trading_mode.value})")
...
def execute_flow_task(flow_id: str, model_provider: str = "groq", model_name: str = None, trading_mode: str = None):
    """
    ...
    Args:
        ...
        trading_mode: Trading mode ("demo" or "real"). If None, determines from flow.
    """
    ...
    async def run():
        ...
        if trading_mode:
            ...
            mode = await get_trading_mode_from_flow(flow_id)
...
def heartbeat_running_executions_task(self: Task) -> Dict[str, Any]:
    ...
    Processes executions from both demo and real databases.
    """
    async def heartbeat_all():
        ...
        for mode in [TradingMode.DEMO, TradingMode.REAL]:
            ...
            # Find all running executions in this database
            ...
            for exec_doc in running_executions:
                ...
                if flow_id:
                    ...
                    if success:
                        total_heartbeat_count += 1
                    else:
                        total_failed_count += 1
        
        return {
            "success": True,
            "heartbeat_count": total_heartbeat_count,
