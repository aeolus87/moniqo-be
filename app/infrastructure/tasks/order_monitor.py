...
Uses OrderRepository instead of direct Beanie access.
"""
...
from app.domain.models.order import Order, OrderStatus, OrderSide, OrderType
...
class OrderMonitorService:
    ...
    async def monitor_order(self, order_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            # Get order from database using repository
    ...
    async def sync_order_from_exchange(self, order: Order) -> Dict[str, Any]:
        ...
        try:
            # Get wallet instance
            wallet_instance = await self.wallet_factory.create_wallet_from_db(
                db=self.db,
    ...
    async def _create_position_from_order(self, order: Order):
        ...
        try:
            ...
            order.position_id = ObjectId(position.id) if isinstance(position.id, str) else position.id
            await self.repository.save(order)
    ...
    async def monitor_user_orders(self, user_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            # Get all open orders for user using repository
            orders = await self.repository.find_open_orders(user_id=user_id)
