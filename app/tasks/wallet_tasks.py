...
def get_async_db() -> AsyncIOMotorDatabase:
    ...
    Note: Can't use FastAPI's Depends in Celery tasks,
    so we create connection directly.
    """
...
def sync_user_wallet_balance(self: Task, user_wallet_id: str, user_id: str) -> Dict[str, Any]:
    """
    ...
    Example:
        # Trigger from code
        sync_user_wallet_balance.delay("wallet_123", "user_456")
        
        # Or synchronous (for testing)
        result = sync_user_wallet_balance("wallet_123", "user_456")
    """
    try:
        logger.info(f"Starting balance sync: wallet={user_wallet_id}")
...
def sync_all_active_wallets(self: Task) -> Dict[str, Any]:
    ...
    Process:
    1. Find all active wallets
    2. Trigger individual sync tasks for each
    3. Return summary
    ...
    try:
        ...
        for wallet in wallets:
            try:
                # Trigger async task (don't wait for result)
...
def test_wallet_connection(self: Task, user_wallet_id: str, user_id: str) -> Dict[str, Any]:
    """
    ...
    Example:
        ...
        test_wallet_connection.delay("wallet_123", "user_456")
    """
    try:
        logger.info(f"Testing wallet connection: wallet={user_wallet_id}")
...
def cleanup_old_sync_logs(self: Task, days: int = 30) -> Dict[str, Any]:
    """
    ...
    Example:
        # Manually trigger
        cleanup_old_sync_logs.delay(days=7)  # Delete logs older than 7 days
    """
    try:
        ...
        logger.info(f"Sync log cleanup complete: deleted {deleted_count} logs")
